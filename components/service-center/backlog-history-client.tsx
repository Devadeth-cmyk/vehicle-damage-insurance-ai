...
interface BacklogHistoryClientProps {
  assessments: AssessmentRecord[];
}

export function BacklogHistoryClient({ assessments }: BacklogHistoryClientProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search history by ID, customer, plate, make..."
            value={searchTerm}
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <option value="ALL">All Records ({assessments.length})</option>
      ...
      {/* ─── History Assessment Records Table ─────────────────────────── */}
      {filteredAssessments.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center shadow-xs">
          ...
          <h3 className="text-sm font-bold text-slate-800">
            {assessments.length === 0 ? "No Historical Assessments Found" : "No Records Matching Search Criteria"}
          </h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 leading-relaxed">
            {assessments.length === 0
              ? "All past assessment intakes generated by your service center will be cataloged here for long-term tracking and audit."
              : "Try refining your keyword query or switching the status filter."}
          </p>
          {assessments.length === 0 && (
            <div className="mt-5">
              <Link
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/70 text-slate-600 text-[11px]">
                  ...
                  <th className="py-3 px-4 font-semibold">Policy Details</th>
                  <th className="py-3 px-4 font-semibold">Photos</th>
                  <th className="py-3 px-4 font-semibold">Date Created</th>
                  <th className="py-3 px-4 font-semibold">Status</th>
                  <th className="py-3 px-4 font-semibold text-right">Record Details</th>
                </tr>
              </thead>
